...
def main():
    ...
    # Train on FF++
    base_dir_train = os.path.join('DataSets', 'f1v2_features', 'ff_plus_plus')
    ...
    print(f"Loading {feature_name} training features from FaceForensics++...")
    ...
    evaluate_dataset(model, "FaceForensics++ (Internal Val)", manifest_path_train, base_dir_train, feature_key, scaler)
    
    # Zero-shot cross-dataset evaluation on Celeb-DF and Deepfakes
    ...
    df_dir = os.path.join('DataSets', 'f1v2_features', 'deepfakes')
    evaluate_dataset(model, "Deepfakes (Zero-shot)", os.path.join(df_dir, 'manifest.csv'), df_dir, feature_key, scaler)
