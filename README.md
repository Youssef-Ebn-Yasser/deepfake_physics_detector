...
Before features are extracted, the raw `.mp4` videos are processed to isolate and normalize the face region. This project supports two preprocessing pipelines:

- **v1 (Baseline)**: Standard face detection using MediaPipe and basic normalization (Gaussian blur).
- **v2 (Advanced)**: Focuses on enhancing micro-details and standardizing the face geometry.
  - **Eye Alignment**: Rotates the face so the eyes are perfectly horizontal, removing head-tilt variances.
  - **CLAHE**: Contrast Limited Adaptive Histogram Equalization to bring out hidden texture details in varying lighting conditions.
  - **Bilateral Filtering**: Smooths noise while preserving sharp edges, which is critical for accurate FFT and biological analysis.

```mermaid
graph TD
    A[Raw Video] --> B[Extract N Frames]
    B --> C[MediaPipe Face Detection]
    C --> D{Preprocessing v2}
    D --> E[Eye Alignment & Rotation]
    E --> F[Crop Face to 256x256]
    F --> G[Apply CLAHE Contrast]
    G --> H[Bilateral Filtering]
    H --> I[Normalized Face Batch]
    I --> J[Feature Extractors]
```
