"""
experiments/eval_cross_dataset_stage_ab.py
-------------------------------------------
Cross-Dataset Zero-Shot Evaluation for Stage A & Stage B Models.

Evaluates EVERY saved model on:
...
Stage A models (one per feature, independent classifiers):
  checkpoints/deepfakes/stage_a/f1/  (Lens Distortion)
  checkpoints/deepfakes/stage_a/f2/  (Motion Blur)
  checkpoints/deepfakes/stage_a/f3/  (Biomechanics)
  checkpoints/deepfakes/stage_a/f4/  (Lighting SH)
  checkpoints/deepfakes/stage_a/f5/  (Frequency FFT)

Stage B models (incremental fusion):
  checkpoints/deepfakes/stage_b/F1_F2/
  checkpoints/deepfakes/stage_b/F1_F2_F3/
  checkpoints/deepfakes/stage_b/F1_F2_F3_F4/
  checkpoints/deepfakes/stage_b/F1_F2_F3_F4_F5/
...
Usage:
    ...
    python experiments/eval_cross_dataset_stage_ab.py

Output:
    checkpoints/deepfakes/cross_dataset/stage_ab_results.json
...
CKPT_A       = os.path.join('checkpoints', 'deepfakes', 'stage_a')
CKPT_B       = os.path.join('checkpoints', 'deepfakes', 'stage_b')
...
# ─────────────────────────────────────────────────────────────────────────────
# Stage A config  (feature_key → model dir)
# ─────────────────────────────────────────────────────────────────────────────

STAGE_A_CONFIGS = [
    {
        'name':         'F1 — Lens Distortion',
        'short':        'StageA-F1',
        'dir':          os.path.join(CKPT_A, 'f1'),
        'feature_keys': ['f1'],
    },
    {
        'name':         'F2 — Motion Blur',
        'short':        'StageA-F2',
        'dir':          os.path.join(CKPT_A, 'f2'),
        'feature_keys': ['f2'],
    },
    {
        'name':         'F3 — Biomechanics',
        'short':        'StageA-F3',
        'dir':          os.path.join(CKPT_A, 'f3'),
        'feature_keys': ['f3'],
    },
    {
        'name':         'F4 — Lighting SH',
        'short':        'StageA-F4',
        'dir':          os.path.join(CKPT_A, 'f4'),
        'feature_keys': ['f4'],
    },
    {
        'name':         'F5 — Frequency FFT',
        'short':        'StageA-F5',
        'dir':          os.path.join(CKPT_A, 'f5'),
        'feature_keys': ['f5'],
    },
]

# ─────────────────────────────────────────────────────────────────────────────
# Stage B config  (fusion group → model dir)
# ─────────────────────────────────────────────────────────────────────────────
...
STAGE_B_CONFIGS = [
    {
        'name':         'F1+F2',
        'short':        'StageB-F1F2',
        'dir':          os.path.join(CKPT_B, 'F1_F2'),
        'feature_keys': ['f1', 'f2'],
    },
    {
        'name':         'F1+F2+F3',
        'short':        'StageB-F1F2F3',
        'dir':          os.path.join(CKPT_B, 'F1_F2_F3'),
        'feature_keys': ['f1', 'f2', 'f3'],
    },
    {
        'name':         'F1+F2+F3+F4',
        'short':        'StageB-F1F2F3F4',
        'dir':          os.path.join(CKPT_B, 'F1_F2_F3_F4'),
        'feature_keys': ['f1', 'f2', 'f3', 'f4'],
    },
    {
        'name':         'F1+F2+F3+F4+F5',
        'short':        'StageB-F1F2F3F4F5',
        'dir':          os.path.join(CKPT_B, 'F1_F2_F3_F4_F5'),
...
def load_cross_dataset_npz(dataset_dir, feature_keys):
    """
    Load .npz files for cross-dataset evaluation.
    Reads manifest.csv if present, otherwise auto-discovers real/ fake/ subdirs.
    Returns X_dict (only requested feature_keys), y.
    """
...
def balance_dataset(X_dict, y, seed=SEED):
    """
    Undersample the majority class so real == fake.
    Returns balanced X_dict, y.
    """
...
def run_stage_a(model, X_norm, feature_key):
    """Stage A model takes a single feature array."""
    X = X_norm[feature_key]
    raw = model.predict(X, verbose=0)
    return raw.flatten()


def run_stage_b(model, X_norm, feature_keys):
    """Stage B model takes features in order [f1, f2, ...] matching its inputs."""
    test_X = [X_norm[k] for k in feature_keys]
...
def evaluate(model, scaler, X_raw, y, feature_keys, stage, run_fn):
    """
    Apply scaler, run model, compute metrics.
    Returns metrics dict.
    """
    X_norm   = scaler.transform(X_raw)
    y_proba  = run_fn(model, X_norm, feature_keys) if stage == 'b' else run_fn(model, X_norm, feature_keys[0])
    metrics  = compute_all_metrics(y, y_proba)
...
def main():
    ...
    # ── Load external datasets (unbalanced first, balance per model) ──────────
    print(f"\n{SEP}")
    ...
    for ds_name, subdir in [
        ...
        if X_raw is None:
            print(f"  [SKIP] {ds_name}: no valid samples found.")
            continue
        print(f"  {ds_name}: {len(y)} samples (real={(y==0).sum()}, fake={(y==1).sum()})")
        X_bal, y_bal = balance_dataset(X_raw, y)
        print(f"    => balanced: {len(y_bal)} samples ({(y_bal==0).sum()} real, {(y_bal==1).sum()} fake)")
        EXT_DS[ds_name] = {'X_raw': X_raw, 'y_raw': y,
                           'X_bal': X_bal, 'y_bal': y_bal}

    # ── Prepare results container ─────────────────────────────────────────────
    all_results = {}
    summary_rows = []   # for the final print table

    # ═════════════════════════════════════════════════════════════════════════
    # Stage A — individual feature models
    # ═════════════════════════════════════════════════════════════════════════
    print(f"\n\n{SEP}")
    print("  STAGE A — INDIVIDUAL FEATURE MODELS")
    print(SEP)
    ...
    for cfg in STAGE_A_CONFIGS:
        ...
        model_results = {'stage': 'A', 'name': name, 'feature': fk, 'datasets': {}}

        # ── DeeperForensics baseline ─────────────────────────────────────────
        if deeper_data:
            X_sub = {fk: deeper_data['X'][fk]}
            y_sub = deeper_data['y']
            X_n   = scaler.transform(X_sub)
            yp    = model.predict(X_n[fk], verbose=0).flatten()
    ...
    # ═════════════════════════════════════════════════════════════════════════
    # Stage B — simple fusion models
    # ═════════════════════════════════════════════════════════════════════════
    print(f"\n\n{SEP}")
    print("  STAGE B — SIMPLE FUSION MODELS")
    print(SEP)
    ...
    for cfg in STAGE_B_CONFIGS:
        ...
        model_results = {'stage': 'B', 'name': name, 'features': fkeys, 'datasets': {}}
        ...
        if deeper_data:
            ...
            y_sub = deeper_data['y']
            X_n   = scaler.transform(X_sub)
            test_X = [X_n[k] for k in fkeys]
            yp    = model.predict(test_X, verbose=0).flatten()
            m     = compute_all_metrics(y_sub, yp)
            model_results['datasets']['DeeperForensics_test'] = m
            summary_rows.append(fmt_row(short, 'DeeperForensics', len(y_sub), m))
            print(f"    DeeperForensics  ROC-AUC={m['roc_auc']:.4f}  Acc={m['accuracy']:.4f}  "
    ...
    # ── Save JSON ─────────────────────────────────────────────────────────────
    out_path = os.path.join(OUT_DIR, 'stage_ab_cross_dataset.json')
    ...
    print(SEP)
    print("""
  Compare ROC-AUC on DeeperForensics vs Celeb-DF vs FF++:

    If a feature's AUC drops from >0.95 (DeeperForensics) to ~0.5 (external):
      => That feature learned DATASET-SPECIFIC patterns, not general physics.

    If a feature's AUC stays >0.7 across all datasets:
      => That feature IS generalizing to unseen deepfake generators.

    Look especially at F1 (Lens Distortion) — it had low internal AUC (0.61)
    but may generalize better than F4/F5 which had near-perfect internal AUC.

    The "internal brilliance" of F4/F5 is the suspect — if they collapse on
    external data, it confirms they overfit to DeeperForensics compression
