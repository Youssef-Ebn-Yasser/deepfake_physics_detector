...
train_master.py
---------------
End-to-end Master Model training (Sub-Systems 1 and 2 only).
...
def transfer_subsystem_weights(master_model, sub1_path=None, sub2_path=None):
    """Transfer pre-trained encoder weights into the master model."""
    if sub1_path and os.path.exists(sub1_path):
        ...
        master_model.get_layer('subsystem1_hardware').set_weights(
    ...
    if sub2_path and os.path.exists(sub2_path):
        ...
        master_model.get_layer('subsystem2_biological').set_weights(
...
def main():
    parser = argparse.ArgumentParser(description='Train Master Fusion Model (Sys 1 + Sys 2)')
    ...
    model.fit(
        train_ds, validation_data=val_ds, epochs=epochs_s1,
        callbacks=[
            tf.keras.callbacks.ModelCheckpoint(
                'checkpoints/master_stage1_best.keras',
                monitor='val_master_logits_acc', save_best_only=True, mode='max', verbose=1,
            ),
        ],
    )
    ...
    model.fit(
        train_ds, validation_data=val_ds, epochs=epochs_s2,
        callbacks=[
            tf.keras.callbacks.ModelCheckpoint(
            ...
            tf.keras.callbacks.EarlyStopping(
                monitor='val_master_logits_acc', mode='max',
