...
from models.master_fusion import (
    build_master_physics_detector,
    SubSystem1Encoder, SubSystem2Encoder, MasterFusionBlock,
)
...
def main():
    ...
    if os.path.exists(args.master):
        ...
        master = tf.keras.models.load_model(
            args.master, compile=False, custom_objects=custom_objs
        )
    else:
    ...
    all_metrics.append(compute_metrics(m_labels, m_probs, 'Master Fusion (Full)'))
    ...
    if os.path.exists(args.sub1):
        sub1 = tf.keras.models.load_model(
            args.sub1, compile=False, custom_objects=custom_objs
        )
    else:
        sub1 = build_subsystem1_model(feature_dim, embed_dim)

    s1_labels, s1_probs = predict_on_dataset(sub1, sub1_ds, 'sub1_logits')
    all_metrics.append(compute_metrics(s1_labels, s1_probs, 'Sub-System 1 (Hardware Optics)'))
    ...
    if os.path.exists(args.sub2):
        sub2 = tf.keras.models.load_model(
            args.sub2, compile=False, custom_objects=custom_objs
        )
    else:
        sub2 = build_subsystem2_model(feature_dim, embed_dim)

    s2_labels, s2_probs = predict_on_dataset(sub2, sub2_ds, 'sub2_logits')
    all_metrics.append(compute_metrics(s2_labels, s2_probs, 'Sub-System 2 (Biological Domain)'))
